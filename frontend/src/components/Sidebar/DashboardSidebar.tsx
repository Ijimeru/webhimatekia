...
const DashboardSidebar = () => {
  ...
  const [activeLocation, setActiveLocation] = useState<string>("/dashboard/");
  ...
  const navLists: { name: string; link: string; icon: React.ReactElement }[] = [
    ...
    { name: "Post Saya", link: "/dashboard/posts/", icon: <BiNews className={className} /> },
    { name: "Buku", link: "/dashboard/books/", icon: <BsBook className={className} /> },
    { name: "Barang Jual", link: "/dashboard/items/", icon: <BsShop className={className} /> },
  ];
  const location = useLocation();
  return (
    <nav className={`bg-[#a33caa] max-h-[100vh] h-[100vh] overflow-x-hidden overflow-y-auto fixed top-0 transition-all w-0  ${sidebarActive ? "w-64" : "md:w-16"} duration-300 z-10`}>
          ...
          {navLists.map((list) => (
            <li className="list-item overflow-hidden w-full" key={list.name}>
              <Link
                to={list.link}
                className={`${(() => {
                  if (list.link === "/dashboard/") {
                    return list.link === location.pathname ? "bg-[rgba(0,0,0,0.25)] text-[#a33caa]" : null;
                  } else {
                    return RegExp(`${list.link}(\w*\W*)*`).test(location.pathname) ? "bg-[rgba(0,0,0,0.25)] text-[#a33caa]" : null;
                  }
                })()} text-white tracking-widest uppercase items-center flex justify-start w-full cursor-pointer`}
                onClick={() => setActiveLocation(list.link)}
              >
